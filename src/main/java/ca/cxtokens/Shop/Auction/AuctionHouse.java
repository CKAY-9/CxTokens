...
public class AuctionHouse {
                ...
                new Runnable() {
                    ...
                    public void run() {
                        if (CxTokens.LOGGING) {
                            tokens
                                .getLogger()
                                .info("Running Auction House sweep...");
                        }
                            ...
                            if (item.sweepsUntilComplete < 0) {
                                ...
                                if (item.bidder == null) {
                                    ...
                                    item.seller.sendMessage(
                                        Utils.formatText(
                                            "&cNo one bought your item on the auction house. Returning..."
                                        )
                                    );
                                    item.seller
                                        .getInventory()
                                    ...
                                    item.seller.sendMessage(
                                        Utils.formatText(
                                            "&aYour item has been sold for &a&l" +
                                                CxTokens.currency +
                                                item.currentBid +
                                                "&r&a on the Auction House!"
                                        )
                                    );
                                    TokenPlayer.getTokenPlayer(
                                        tokens,
                                        item.seller
                                    ).addTokens(item.currentBid, true);

                                    item.bidder.sendMessage(
                                        Utils.formatText(
                                            "&aSuccessfully bought an item off the Auction House for &a&l" +
                                                CxTokens.currency +
                                                item.currentBid
                                        )
                                    );
                        ...
                        if (CxTokens.LOGGING) {
                            tokens
                                .getLogger()
                                .info("Finished Auction House sweep!");
                        }
    ...
    public void openAuctionHouse(Player player, int pageIndex) {
        Inventory auctionInv = Bukkit.createInventory(
            null,
            54,
            Utils.formatText("&a&lAuction House - Page " + (pageIndex + 1))
        );
        auctionInv.clear();
        ...
        if (
            this.auctionItems.size() < GlobalShop.MAX_ITEMS_PER_PAGE ||
        ...
        for (int i = 0 + start; i < limit; i++) {
            ...
            lore.add(
                Utils.formatText(
                    "&aCurrent Bid: &l" + CxTokens.currency + temp.currentBid
                )
            );
            lore.add(Utils.formatText("&aSeller: &l" + temp.seller.getName()));
            lore.add(
                Utils.formatText(
                    "&aTime Remaining: &l" + temp.sweepsUntilComplete + "m"
                )
            );
            lore.add(
                Utils.formatText(
        ...
        if (pageIndex == 0) {
            ...
            boeMeta.setDisplayName(
                Utils.formatText("&cBack to page " + pageIndex)
            );
        }
        ...
        if (
            pageIndex <
            Math.round(this.auctionItems.size() / GlobalShop.MAX_ITEMS_PER_PAGE)
        ) {
            ItemStack next = new ItemStack(Material.GREEN_CONCRETE, 1);
            ItemMeta nextMeta = next.getItemMeta();
            nextMeta.setDisplayName(
                Utils.formatText("&aGo to page " + (pageIndex + 2))
            ); // + 2 is just so the pages go 1, 2, 3,... instead of 0, 1, 2,...
    ...
    public void sellItemOnHouse(TokenPlayer player, long sellPrice) {
        if (
            player.ply.getInventory().getItemInMainHand().getType() ==
            Material.AIR
        ) {
            player.ply.sendMessage(
                Utils.formatText(
                    "&cYou need to have an item in your main hand to sell!"
                )
            );
            return;
        }
        ...
        if (sellPrice < 0) {
            player.ply.sendMessage(
                Utils.formatText("&cItems cannot have a negative price!")
            );
            return;
        }

        // .clone() is needed as removing the item sets the auction item to be AIR
        ItemStack itemToSell = player.ply
            .getInventory()
            .getItemInMainHand()
            .clone();
        ...
        player.ply.sendMessage(
            Utils.formatText("&aYour item is now on the Auction House!")
        );
