...
public class AdminCommand implements CommandExecutor {
    ...
    public boolean onCommand(
        ...
        if (!sender.isOp()) {
            player.sendMessage(
                Utils.formatText(
                    "&cYou need to be an operator to run this command!"
                )
            );
            return false;
        }
        ...
        if (args.length <= 0) {
            ...
            sender.sendMessage(
                Utils.formatText(
                    "&c  <set/add/subtract/reset> <player> <number>"
                )
            );
            return false;
        }
        ...
        try {
            if (!args[0].equalsIgnoreCase("config")) {
                ...
                player.sendMessage(
                    Utils.formatText("&aSuccessfully executed command!")
                );
            } else {
                // Config handler
                ...
                player.sendMessage(
                    Utils.formatText("&aSuccessfully updated config!")
                );
            }
        } catch (Exception ex) {
            if (CxTokens.LOGGING) {
